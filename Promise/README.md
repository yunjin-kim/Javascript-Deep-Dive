...
그 이유는 프로미스의 후속 처리 메서드의 콜백 함수는 태스크 큐가 아니라 마이크로태스크 큐로 저장되기 때문이다
...
이후 마이크로태스크 큐에서 대기하고 있는 함수를 가져와 실행한다
...
```js
fetch('https://example.com/user/info')
...
두번째 인수로 HTTP 요청 메서드, HTTP 요청 헤더, 페이로드 등을 설정한 객체를 전달한다
```js
const request = {
  get(url) {
    return fetch(url);
